...
const FEN_MAP: Record<string, [PieceType, Side]> = {
...
const REV_FEN_MAP: Record<string, string> = {
...
export class FenCodec {
  static encode(board: (Piece | null)[][]): string {
    const rows: string[] = [];
    for (let y = 0; y < 10; y++) {
      let row = '';
      let empty = 0;
  ...
  static decode(fen: string): { board: (Piece | null)[][]; side: Side } | null {
