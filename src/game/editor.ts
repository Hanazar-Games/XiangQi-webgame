import { Piece, Position, Side } from './types.js';
import { Rules } from './rules.js';

export type Brush = Piece | 'erase' | null;
...
export function createStandardBoard(): (Piece | null)[][] {
  const board = createEmptyBoard();
  const blackPieces: [number, number, Piece['type']][] = [
  ...
  const redPieces: [number, number, Piece['type']][] = [
  ...
  for (const [x, y, type] of blackPieces) board[y][x] = { type, side: 'black' };
  for (const [x, y, type] of redPieces) board[y][x] = { type, side: 'red' };
  return board;
}
...
export function placePiece(board: (Piece | null)[][], pos: Position, piece: Piece | null): void {
...
const VALID_TYPES: Piece['type'][] = ['king', 'advisor', 'elephant', 'horse', 'rook', 'cannon', 'pawn'];
const VALID_SIDES: Side[] = ['red', 'black'];

export function validateBoard(board: (Piece | null)[][]): ValidationResult {
  const errors: string[] = [];
  let redKing = 0;
  let blackKing = 0;
  ...
  if (redKing === 0) errors.push('红方缺少帅');
  else if (redKing > 1) errors.push(`红方有 ${redKing} 个帅，只能有1个`);

  if (blackKing === 0) errors.push('黑方缺少将');
  else if (blackKing > 1) errors.push(`黑方有 ${blackKing} 个将，只能有1个`);
  ...
  if (redKingPos && blackKingPos) {
    if (redKingPos.x === blackKingPos.x) {
      ...
      for (let y = minY + 1; y < maxY; y++) {
        if (board[y][redKingPos.x]) { clear = false; break; }
      }
      if (clear) errors.push('将帅不能直接照面');
    }
  }
...
export const BRUSH_PIECES: { side: Side; type: Piece['type']; label: string }[] = [
