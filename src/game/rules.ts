...
export class Rules {
  ...
  private static kingsFaceEachOther(board: (Piece | null)[][], pos: Position, side: Side): boolean {
    ...
    for (let y = 0; y < 10; y++) {
      ...
      if (piece && piece.type === 'king' && piece.side === enemySide) {
        ...
        for (let yy = minY + 1; yy < maxY; yy++) {
          if (board[yy][pos.x]) return false;
        }
        return true;
      }
    }
  ...
  private static isValidKingMove(
    ...
    // 不能飞将
    if (this.kingsFaceEachOther(board, to, side)) return false;

    return true;
  }

  private static isValidAdvisorMove(
    from: Position,
    to: Position,
    side: Side
  ): boolean {
  ...
  private static isValidElephantMove(
    board: (Piece | null)[][],
    from: Position,
    to: Position,
    side: Side
  ...
  private static isValidHorseMove(
    board: (Piece | null)[][],
    from: Position,
    to: Position
  ): boolean {
  ...
  private static isValidCannonMove(
    board: (Piece | null)[][],
    from: Position,
    to: Position
  ): boolean {
  ...
  // 检测某一方是否被将军
  static isInCheck(board: (Piece | null)[][], side: Side): boolean {
  ...
  private static isValidMoveWithoutCheck(
    board: (Piece | null)[][],
    from: Position,
    to: Position
  ): boolean {
    ...
    switch (piece.type) {
      case 'king': {
        if ((dx === 1 && dy === 0) || (dx === 0 && dy === 1)) {
          ...
          // 不能飞将
          if (this.kingsFaceEachOther(board, to, piece.side)) return false;
          return true;
        }
      ...
      case 'elephant': {
        if (dx !== 2 || dy !== 2) return false;
        if (piece.side === 'red' && to.y < 5) return false;
        if (piece.side === 'black' && to.y > 4) return false;
        const eyeX = (from.x + to.x) / 2;
        const eyeY = (from.y + to.y) / 2;
