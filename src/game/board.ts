import { Piece, Position, Side, Move, GameState, PIECE_NAMES } from './types.js';
...
export class Board {
  ...
  private createInitialState(): GameState {
    ...
    // 黑方（上方）
    const blackPieces: [number, number, PieceType][] = [
    ...
    // 红方（下方）
    const redPieces: [number, number, PieceType][] = [
  ...
  makeMove(from: Position, to: Position): boolean {
    ...
    if (!Rules.isValidMove(this.state.board, from, to)) return false;
    if (Rules.wouldBeInCheck(this.state.board, from, to, piece.side)) return false;
  ...
  loadCustomBoard(board: (Piece | null)[][], startingSide: Side): void {
    this.state = {
  ...
  // 序列化移动（用于网络传输）
  static serializeMove(move: Move): string {
  ...
  static deserializeMove(data: string): Move {
