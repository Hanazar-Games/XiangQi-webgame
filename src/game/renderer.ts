import { Piece, Position, Side, GameState, Move, PIECE_NAMES } from './types.js';
import { Animator } from './animation.js';
import { ParticleSystem } from './particles.js';
import { Theme, CLASSIC_THEME } from './themes.js';
...
export class Renderer {
  ...
  // 将棋盘坐标转换为canvas坐标
  private toCanvas(pos: Position): { x: number; y: number } {
  ...
  // 将canvas坐标转换为棋盘坐标
  toBoard(canvasX: number, canvasY: number): Position | null {
  ...
  render(
    ...
    for (let y = 0; y < 10; y++) {
      for (let x = 0; x < 9; x++) {
        if (animSet && animSet.has(`${x},${y}`)) continue;
  ...
  private drawPiece(pos: Position, piece: Piece): void {
    const { x, y } = this.toCanvas(pos);
    this.drawPieceAt(x, y, piece);
  }

  private drawPieceAt(x: number, y: number, piece: Piece): void {
  ...
  private drawHighlight(pos: Position, color: string): void {
    const { x, y } = this.toCanvas(pos);
  ...
  private drawMoveIndicator(pos: Position): void {
    const { x, y } = this.toCanvas(pos);
    const { ctx } = this;
  ...
  private drawHintArrow(move: Move): void {
  ...
  private findKing(state: GameState): Position | null {
    for (let y = 0; y < 10; y++) {
      for (let x = 0; x < 9; x++) {
        const piece = state.board[y][x];
  ...
  triggerCaptureEffect(pos: Position, side: Side): void {
    const { x, y } = this.toCanvas(pos);
