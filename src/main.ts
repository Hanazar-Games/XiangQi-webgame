...
import { Storage, PuzzleRecord, ResumeState } from './game/storage.js';
...
class GameApp {
  ...
  private animator = new Animator(() => this.renderBoard());
  private notation = new Notation();
  private difficulty: Difficulty = 'normal';
  private handicap: 'none' | 'horse' | 'cannon' | 'rook' = 'none';
  private showEvaluation = false;
  ...
  private setupEventListeners(): void {
    ...
    document.querySelectorAll('.diff-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        this.difficulty = (e.currentTarget as HTMLElement).dataset.diff as Difficulty;
        this.saveSettings();
      });
    });
    ...
    document.querySelectorAll('.handicap-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        (e.currentTarget as HTMLElement).classList.add('active');
        this.handicap = (e.currentTarget as HTMLElement).dataset.handicap as typeof this.handicap;
      });
    });
    ...
    (document.getElementById('ai-speed') as HTMLInputElement).addEventListener('input', (e) => {
    ...
    // 坐标显示
    const coordsToggle = document.getElementById('coords-toggle') as HTMLInputElement;
    ...
    // 评估显示
    const evalToggle = document.getElementById('eval-toggle') as HTMLInputElement;
    if (evalToggle) {
    ...
    document.getElementById('history-list')!.addEventListener('click', (e) => {
      const btn = (e.target as HTMLElement).closest('.btn-analysis') as HTMLButtonElement | null;
    ...
    document.getElementById('puzzle-list')!.addEventListener('click', (e) => {
      ...
      const deleteBtn = target.closest('.puzzle-delete-btn') as HTMLElement | null;
      if (deleteBtn) {
        e.stopPropagation();
      ...
      const card = target.closest('.puzzle-card') as HTMLElement | null;
      if (card) {
  ...
  private checkResumeState(): void {
    ...
    if (!resume || resume.mode !== 'lan') return;
  ...
  private loadSettings(): void {
    ...
    document.querySelectorAll('.diff-btn').forEach(btn => {
      btn.classList.toggle('active', (btn as HTMLElement).dataset.diff === this.difficulty);
    });
  }
  ...
  private restartGame(): void {
    ...
    if (this.mode !== 'lan-join') {
      ...
      (document.getElementById('flip-toggle') as HTMLInputElement).checked = false;
    }
    this.updateGameInfo();
    this.renderBoard();
    this.updateCaptured();
  ...
  private startGame(mode: GameMode): void {
    ...
    if (mode === 'local-ai') {
      ...
      (document.getElementById('flip-toggle') as HTMLInputElement).checked = true;
    } else {
      this.mySide = null;
    }
    this.rememberInitialPosition();
  ...
  private async showLanHost(): Promise<void> {
    ...
    (document.getElementById('host-offer') as HTMLTextAreaElement).value = '正在生成...';
    document.getElementById('lan-status')!.textContent = '';

    this.p2p = new P2PConnection(
      (data) => this.onP2PMessage(data),
      (state) => this.onP2PStateChange(state)
    );
    ...
    try {
      ...
      document.getElementById('lan-status')!.textContent = '生成失败: ' + (e as Error).message;
    }
  }

  private async hostConnect(): Promise<void> {
    if (!this.p2p) return;
    const answer = (document.getElementById('host-answer') as HTMLTextAreaElement).value.trim();
    if (!answer) {
    ...
    try {
      ...
      document.getElementById('lan-status')!.textContent = '连接失败: ' + (e as Error).message;
    }
  }
  ...
  private async joinGetAnswer(): Promise<void> {
    if (!this.p2p) return;
    const offer = (document.getElementById('join-offer') as HTMLTextAreaElement).value.trim();
    if (!offer) {
  ...
  private copyText(elementId: string): void {
  ...
  private sendChat(): void {
    ...
    if (!text || !this.p2p?.isConnected()) return;
    this.p2p.send(wrapMessage('chat', text));
    this.receiveChat(text, true);
    input.value = '';
  }

  private receiveChat(text: string, isSelf: boolean): void {
    const container = document.getElementById('chat-messages')!;
    const div = document.createElement('div');
  ...
  private jumpToMove(index: number): void {
  ...
  private reviewStep(delta: number): void {
    if (this.reviewIndex === null) return;
    const entries = this.notation.getAll();
  ...
  private resign(): void {
    if (this.board.state.gameOver || !this.mySide || this.reviewIndex !== null) return;
  ...
  private startTimer(): void {
    if (this.timerInterval || this.board.state.gameOver || this.reviewIndex !== null) return;
